...
import { SponsorDialog } from "@/components/crm/SponsorDialog";
import { DeleteButton } from "@/components/crm/DeleteButton";

const proposalLabel: Record<string, string> = { draft: "下書き", sent: "送付済み", negotiating: "交渉中", agreed: "合意", rejected: "不成立" };
const contractLabel: Record<string, string> = { none: "未締結", sent: "送付済み", signed: "締結済み", cancelled: "解除" };
const invoiceLabel: Record<string, string> = { none: "未請求", issued: "請求済み", partial: "一部入金", paid: "入金済み", overdue: "延滞" };
const typeLabel: Record<string, string> = { sponsor: "スポンサー", media: "メディア", government: "自治体", school: "学校", ngo: "団体", other: "その他" };

export default async function ProjectCrmPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <CompanyDialog />
          <SponsorDialog companies={companiesForSelect} />
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>{pct}% 達成</span><span>目標: ¥25,000,000</span>
          </div>
        </CardContent>
      </Card>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {!sponsorships || sponsorships.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-12 text-muted-foreground">協賛が登録されていません</td></tr>
              ) : (
                sponsorships.map((s) => (
                  <tr key={s.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3"><Badge variant={s.proposal_status === "agreed" ? "default" : "secondary"}>{proposalLabel[s.proposal_status]}</Badge></td>
                    <td className="px-4 py-3"><Badge variant={s.contract_status === "signed" ? "default" : "outline"}>{contractLabel[s.contract_status]}</Badge></td>
                    <td className="px-4 py-3"><Badge variant={s.invoice_status === "paid" ? "default" : "outline"}>{invoiceLabel[s.invoice_status]}</Badge></td>
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <SponsorDialog companies={companiesForSelect} sponsorship={s} trigger={<Button size="sm" variant="ghost" className="h-7 text-xs">編集</Button>} />
                        <DeleteButton type="sponsorship" id={s.id} name={(s.crm_companies as any)?.name} />
                      </div>
                    </td>
                  </tr>
                ))
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {!companies || companies.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-12 text-muted-foreground">企業が登録されていません</td></tr>
              ) : (
                companies.map((c) => (
                  <tr key={c.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <CompanyDialog company={c} trigger={<Button size="sm" variant="ghost" className="h-7 text-xs">編集</Button>} />
                        <DeleteButton type="company" id={c.id} name={c.name} />
                      </div>
                    </td>
                  </tr>
                ))
